...
def preprocess_image_tf(image):
    ...
    MobileNetV2 espera imagens RGB (3 canais), então adaptamos o shape se necessário.
    """
    ...
    # Redimensionar para 224x224 (tamanho padrão de entrada da MobileNetV2)
...
def build_model_with_mobilenet_v2(input_shape, lr, l2_regularizer=1e-5, freeze_base=True):
    """
    Constrói um modelo usando MobileNetV2 pré-treinada com transfer learning.
    
    Parâmetros:
    -----------
    input_shape : tuple
        Shape da imagem de entrada (224, 224, 3) para MobileNetV2
    ...
    freeze_base : bool
        Se True, congela os pesos da MobileNetV2 pré-treinada (transfer learning)
    ...
    1. CARREGAMENTO DA MOBILENETV2 PRÉ-TREINADA:
       - MobileNetV2 foi treinada no dataset ImageNet com ~1.2 milhões de imagens
       - Arquitetura otimizada para dispositivos móveis (5.3M parâmetros vs 25.6M do ResNet50)
       - Usa convolução separável depthwise para eficiência
       - Já aprendeu a extrair features visuais (bordas, texturas, padrões complexos)
    
    2. REMOÇÃO DO "HEAD" DE CLASSIFICAÇÃO:
       - A MobileNetV2 original tem 1000 saídas (classes ImageNet)
    ...
    3. CONGELAMENTO DOS PESOS (TRANSFER LEARNING):
       ...
       - Vantagem: treinamento MUITO mais rápido que ResNet (4x menos parâmetros)
       - Mantém o conhecimento de features do ImageNet
    
    4. ADIÇÃO DE CAMADAS CUSTOMIZADAS (HEAD):
       - Flatten: reduz o output da MobileNetV2 de (7,7,1280) para (62720,)
    ...
    5. POR QUE MOBILENETV2 PARA CICLONES:
       - MobileNetV2 é mais leve e rápido que ResNet50 (menos parâmetros)
       - Mantém qualidade similar em transfer learning
       - Ideal para Kaggle (menos memória, menos tempo)
       - Ciclones têm padrões visuais que MobileNetV2 consegue capturar bem
    
    ============================================================================
    """
    
    # ========== CARREGA MOBILENETV2 PRÉ-TREINADA ==========
    ...
    # Se freeze_base=True, não atualiza os pesos da MobileNetV2 durante treinamento
    ...
    if freeze_base:
        print("✓ Pesos da MobileNetV2 congelados (Transfer Learning)")
    else:
        print("✓ Pesos da MobileNetV2 descongelados (Fine-tuning completo)")
    
    # ========== CONSTRÓI MODELO SEQUENCIAL COM MOBILENETV2 COMO BASE ==========
    model = keras.models.Sequential([
        ...
        # Flatten: converte (7, 7, 1280) -> (62720,)
...
def main(
    ...
    """
    Executar treinamento da MobileNetV2 com Transfer Learning
    ...
    freeze_base : bool, default=True
        Congelar pesos da MobileNetV2 (Transfer Learning)
    """
    ...
    # Constrói modelo com MobileNetV2
    model = build_model_with_mobilenet_v2(
        (224, 224, 3),  # MobileNetV2 sempre usa 224x224x3
        learning_rate,
        l2_regularizer,
